...
**Vostride** | [github.com/vostride/agent-qa](https://github.com/vostride/agent-qa)

Специализированный тестовый агент для web- и mobile-приложений. В отличие от универсальных coding agents, Agent QA запускает описанные на естественном языке действия и проверки через CLI или MCP-сервер. Он сохраняет шаги, скриншоты, логи и результаты запуска, а также использует память предыдущих запусков при последующих проверках.
...
Текущие версии распространяются по source-available лицензии FSL-1.1-ALv2, а не по OSI-approved open-source лицензии. Каждая версия переходит на Apache-2.0 через два года после публикации. За разрешённое использование самого Agent QA плата не взимается; отдельно могут оплачиваться подключённые модели, браузерные или device-провайдеры.
